...
class ExtractiveSummarizer(pl.LightningModule):
    ...
    def __init__(self, hparams, embedding_model_config=None, classifier_obj=None):
        ...
        self.sbert = SentenceTransformer("sentence-transformers/all-mpnet-base-v2")
    ...
    def ensure_extractiveness(self, summary, dialog_formatted):
        ...
        encoded_dialogs = np.array([self.sbert.encode(l) for l in split_dialogs])
        encoded_summary_lines = np.array([self.sbert.encode(l) for l in split_summary])

        closest_line_ids = list(
            set(
                util.cos_sim(encoded_dialogs, encoded_summary_lines).argmax(-1).tolist()
            )
        )
